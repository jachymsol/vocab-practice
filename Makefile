...
run-https:
	anvil-app-server --app ${APP_NAME} --origin ${ORIGIN} --data-dir ${DATA_DIR} --dep-id C6ZZPAPN4YYF5NVJ=${ANVIL_EXTRAS_LOCATION} --http-redirect-port 80 --letsencrypt-storage ${LETSENCRYPT_STORAGE}
...
run-https-with-email:
	...
	anvil-app-server --app ${APP_NAME} --origin ${ORIGIN} --data-dir ${DATA_DIR} --dep-id C6ZZPAPN4YYF5NVJ=${ANVIL_EXTRAS_LOCATION} --http-redirect-port 80 --letsencrypt-storage ${LETSENCRYPT_STORAGE} --smtp-host ${SMTP_HOST} --smtp-port ${SMTP_PORT} --smtp-encryption ${SMTP_ENCRYPTION} --smtp-username ${SMTP_USERNAME} --smtp-password ${SMTP_PASSWORD}
